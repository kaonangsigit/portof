...
export default function Navigation() {
  ...
  // ── Navigate to section — works on all platforms including iOS Safari ────
  function navigateTo(href: string) {
    // Close menu first
    setIsMenuOpen(false);
    const id = href.replace("#", "");
    // Small timeout so menu close animation doesn't interfere with scroll
    setTimeout(() => {
  ...
  return (
    <>
      ...
      >
        <nav className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8" aria-label="Main navigation">
          <div className="flex items-center justify-between h-16">
            ...
            <button
              type="button"
              className="md:hidden p-2.5 rounded-lg text-gray-400 hover:text-white
      ...
      {/* ── Mobile menu — OUTSIDE header, full width overlay ─────────────────
          Placed outside the header element so it is NOT clipped by any
          overflow:hidden or transform context. Uses CSS transition only
          (no Framer Motion) for maximum compatibility with iOS Safari.
      ──────────────────────────────────────────────────────────────────── */}
      <div
        id="mobile-menu"
        role="dialog"
        ...
        className={cn(
          ...
          "bg-[#020817] border-b border-white/8 shadow-xl",
      ...
      >
        <ul className="px-4 py-3 space-y-1" role="list">
          {navLinks.map((link) => {
            ...
            return (
              <li key={link.href}>
                {/* Using <button> not <a> to guarantee touch events fire on iOS */}
                <button
                  type="button"
                  onClick={() => navigateTo(link.href)}
                  className={cn(
                    "w-full text-left px-4 py-3.5 text-sm font-medium rounded-xl",
                    "transition-colors duration-150",
                    isActive
                      ? "text-blue-400 bg-blue-500/15 border border-blue-500/25"
                      : "text-gray-300 hover:text-white hover:bg-white/8 active:bg-white/12"
                  )}
                >
                  {link.label}
                </button>
